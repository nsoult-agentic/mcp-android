...
const EMULATOR_SYSIMAGE =
  process.env["EMULATOR_SYSIMAGE"] || "system-images;android-35;google_apis;x86_64";
...
// ── Validation ─────────────────────────────────────────────
// Pure input validators (package/activity/serial/avd-name), the ADB-shell
// allowlist parser, the rate limiter, secret redaction, port parsing, and text
// encoding live in ./validation.ts. The path validators below stay here: they
// call realpathSync and so need a live filesystem.
...
// F10: Reject path traversal on device paths (delegates to the pure prefix matcher).
function validatePullDevicePath(devicePath: string): void {
  validatePullDevicePathPrefixes(devicePath, ALLOWED_PULL_PREFIXES);
}
...
function validatePullLocalPath(localPath: string): string {
  ...
  if (
    !realParent.startsWith(ALLOWED_PULL_LOCAL_DIR) ||
    (realParent !== ALLOWED_PULL_LOCAL_DIR && !realParent.startsWith(`${ALLOWED_PULL_LOCAL_DIR}/`))
  ) {
...
// Ensure a stable adb key resolvable via $HOME so adb authorizes the (managed) emulator headless
// (rootless keep-id "device unauthorized" fix — SB #2424). Idempotent; non-fatal on failure — adb/GMD
// may still auto-generate, but a failure here (e.g. $HOME not writable under rootless keep-id) is the
// exact cause of a downstream "device unauthorized", so it is logged. Returns the resolved key path.
async function ensureAdbKey(): Promise<string> {
  ...
  if (!existsSync(adbKeyPath)) {
    try {
      ...
      try {
        chmodSync(adbKeyPath, 0o600);
      } catch {
        /* best-effort: key is owner-only */
      }
    } catch (e) {
      console.error(
        `adbkey ensure failed (non-fatal): ${e instanceof Error ? e.message : String(e)}`,
      );
    }
  }
  return adbKeyPath;
...
async function adbKnownPorts(): Promise<Set<number>> {
  try {
    ...
    return parseAdbDevicePorts(stdout);
  } catch {
    /* adb not up yet — only our owned ports matter for allocation */
...
async function pickEmulatorPort(): Promise<number> {
  return pickEmulatorPortPure(await usedEmulatorPorts());
}
...
async function avdExists(name: string): Promise<boolean> {
  const { stdout } = await execFile(AVDMANAGER_PATH, ["list", "avd", "-c"], {
    timeout: 30_000,
    env: emulatorEnv(),
  });
  return stdout
...
async function createAvd(name: string): Promise<void> {
  await new Promise<void>((resolveP, rejectP) => {
    const proc = spawn(
      AVDMANAGER_PATH,
      ["create", "avd", "-n", name, "-k", EMULATOR_SYSIMAGE, "-d", EMULATOR_DEVICE, "--force"],
      { env: emulatorEnv() },
    );
    const timer = setTimeout(() => {
      proc.kill("SIGKILL");
      rejectP(new Error("avdmanager create timed out after 60s"));
    }, 60_000);
    let err = "";
...
function createServer(): McpServer {
  ...
  const EmulatorStartInput = {
    avd_name: z
      ...
      .min(1)
      .max(64)
      .default(EMULATOR_AVD)
      .describe("AVD to boot (created from the default system image if missing)"),
  ...
  server.tool(
    ...
    async ({ avd_name, cold_boot }) => {
      ...
      if (startInFlight) {
        const text = await startInFlight.catch(
          (e) => `previous start failed: ${e instanceof Error ? e.message : String(e)}`,
        );
      ...
      try {
        ...
        return {
          content: [
            {
              type: "text" as const,
              text: `FAIL: ${err instanceof Error ? err.message : String(err)}`,
            },
          ],
        };
      } finally {
  ...
  const EmulatorStopInput = {
    device_serial: z
      .string()
      .min(1)
      .max(64)
      .describe("Emulator serial returned by android-emulator-start"),
  };
  server.tool(
    ...
    async ({ device_serial }) => {
      ...
      if (!/^emulator-\d+$/.test(device_serial))
        throw new Error("android-emulator-stop only operates on emulator-* serials");
      if (!ownedEmulators.has(device_serial)) {
        const owned = [...ownedEmulators.keys()].join(", ") || "none";
        throw new Error(
          `Refusing to stop ${device_serial}: not started by this server (owned: ${owned}). Use the originating session, or kill it manually.`,
        );
      }
      ...
      return {
        content: [{ type: "text" as const, text: `Stop ${device_serial}: ${result || "killed"}` }],
      };
    },
  );
    ...
    async ({ device_serial, apk_path, package_name, activity_name }) => {
      ...
      if (pid.length === 0) {
        ...
        const fatalLines = crashLog
          .split("\n")
          .filter(
            (l) => l.includes("FATAL") || l.includes("AndroidRuntime") || l.includes(package_name),
          )
          .join("\n");
        return {
          content: [
    ...
    async () => {
      try {
        ...
        if (builds.length === 0) {
          return {
            content: [
              { type: "text" as const, text: "No APK or AAB files found in /data/builds/" },
            ],
          };
        }
        ...
        return {
          content: [
            { type: "text" as const, text: `## Builds (${builds.length})\n${builds.join("\n")}` },
          ],
        };
      } catch (err: unknown) {
        const msg = err instanceof Error ? err.message : String(err);
        return {
          content: [{ type: "text" as const, text: `Error listing builds: ${msg.slice(0, 200)}` }],
        };
      }
    },
  );
  ...
  server.tool(
    ...
    async ({ device_serial }) => {
      try {
        ...
        const msg = err instanceof Error ? err.message : String(err);
        return {
          content: [{ type: "text" as const, text: `Screenshot failed: ${msg.slice(0, 200)}` }],
        };
      }
    },
  );
  ...
  const InstrumentedTestInput = {
    ...
    device: z
      ...
      .default("aospAtd")
      .describe(
        "GMD managed-device name as declared in the module's testOptions.managedDevices (default: aospAtd)",
      ),
    gradle_module: z
      .string()
      .min(1)
    ...
    gpu: z
      ...
      .default("host")
      .describe(
        "Emulator GPU mode for GMD (default: host — required on this AMD iGPU; swiftshader software-Vulkan crashes on RADV)",
      ),
  };
  server.tool(
  ...
  server.tool(
    ...
    async ({ job_id }) => {
      ...
      const statusText = statusObj
        ...
        : "(unreadable status file)";
      let logTail = "(no log yet)";
      try {
        logTail = redactSensitive(readFileSync(logPath, "utf8")).slice(-4000);
      } catch {
        /* no log yet */
      }
      return {
    ...
    async ({ directory }) => {
      try {
        ...
        if (base !== ALLOWED_INSTALL_DIR && !base.startsWith(`${ALLOWED_INSTALL_DIR}/`)) {
          return {
            content: [{ type: "text" as const, text: "Error: path outside /data/builds/" }],
          };
        }
        ...
        const msg = err instanceof Error ? err.message : String(err);
        return {
          content: [{ type: "text" as const, text: `Error listing files: ${msg.slice(0, 200)}` }],
        };
      }
    },
  );
  ...
  server.tool(
    ...
    "Download a file from /data/builds/ as base64. Use for transferring screenshots or APKs to other services (e.g., Nextcloud). Max 10MB.",
    {
      path: z
        ...
        .max(300)
        .describe(
          "File path relative to /data/builds/ (e.g., 'screenshots/emulator-5556_2026-04-30.png')",
        ),
    },
    async ({ path: filePath }) => {
      try {
        ...
        if (st.size > MAX_DOWNLOAD_BYTES) {
          ...
          return {
            content: [
              { type: "text" as const, text: `Error: file too large (${sizeMB} MB, max 10 MB)` },
            ],
          };
        }
        ...
        const msg = err instanceof Error ? err.message : String(err);
        return {
          content: [{ type: "text" as const, text: `Download failed: ${msg.slice(0, 200)}` }],
        };
      }
    },
  );
  ...
  server.tool(
    ...
    "Tap at screen coordinates. Use android-ui-dump to find element bounds.",
    {
      device_serial: z
        ...
        .max(64)
        .describe("Device serial (from android-list-devices)"),
      x: z.number().int().min(0).max(4096).describe("X coordinate"),
      y: z.number().int().min(0).max(4096).describe("Y coordinate"),
    },
  ...
  server.tool(
    ...
    "Swipe gesture between two points. Duration in milliseconds.",
    {
      device_serial: z
        ...
        .max(64)
        .describe("Device serial (from android-list-devices)"),
      x1: z.number().int().min(0).max(4096).describe("Start X coordinate"),
      y1: z.number().int().min(0).max(4096).describe("Start Y coordinate"),
      x2: z.number().int().min(0).max(4096).describe("End X coordinate"),
      y2: z.number().int().min(0).max(4096).describe("End Y coordinate"),
      duration_ms: z
        .number()
        .int()
        .min(100)
      ...
      try {
        ...
        return {
          content: [
            {
              type: "text" as const,
              text: result || `Swiped (${x1},${y1}) → (${x2},${y2}) in ${duration_ms}ms`,
            },
          ],
        };
      } catch (err: unknown) {
  ...
  server.tool(
    ...
    "Type text into the focused input field. Spaces and special characters are escaped automatically.",
    {
      device_serial: z
        ...
        .max(64)
        .describe("Device serial (from android-list-devices)"),
      text: z.string().min(1).max(500).describe("Text to type into the focused field"),
    },
    async ({ device_serial, text }) => {
      try {
        ...
        return {
          content: [{ type: "text" as const, text: result || `Typed ${text.length} characters` }],
        };
      } catch (err: unknown) {
        const msg = err instanceof Error ? err.message : String(err);
        return {
          content: [{ type: "text" as const, text: `Input text failed: ${msg.slice(0, 200)}` }],
        };
      }
    },
  );
  ...
  server.tool(
    ...
    "Send a key event. Only safe keycodes are allowed (no POWER, SLEEP, REBOOT).",
    {
      ...
      keycode: z
        .enum(KEYCODE_NAMES)
        .describe(
          "Key name: BACK, HOME, ENTER, TAB, DPAD_UP, DPAD_DOWN, DPAD_LEFT, DPAD_RIGHT, DEL, FORWARD_DEL, ESCAPE, APP_SWITCH, MENU, SPACE, MOVE_HOME, MOVE_END",
        ),
    },
    async ({ device_serial, keycode }) => {
      try {
        ...
        return {
          content: [
            { type: "text" as const, text: result || `Sent keyevent ${keycode} (${code})` },
          ],
        };
      } catch (err: unknown) {
        const msg = err instanceof Error ? err.message : String(err);
        return {
          content: [{ type: "text" as const, text: `Keyevent failed: ${msg.slice(0, 200)}` }],
        };
      }
    },
  );
  ...
  server.tool(
    ...
    async ({ device_serial }) => {
      ...
      try {
        ...
        const msg = err instanceof Error ? err.message : String(err);
        return {
          content: [{ type: "text" as const, text: `UI dump failed: ${msg.slice(0, 200)}` }],
        };
      } finally {
  ...
  server.tool(
    ...
    async ({ path: filePath }) => {
      try {
        if (!FILE_RECEIVE_URL) {
          return {
            content: [{ type: "text" as const, text: "Error: FILE_RECEIVE_URL not configured" }],
          };
        }
        ...
        if (!res.ok) {
          ...
          return {
            content: [
              {
                type: "text" as const,
                text: `Push failed (${res.status}): ${errBody.slice(0, 200)}`,
              },
            ],
          };
        }
  ...
  server.tool(
      ...
      "All other commands are rejected.",
    {
      device_serial: z
        ...
        .max(64)
        .describe("Device serial (from android-list-devices)"),
      command: z.string().min(1).max(500).describe("Shell command to run (must match allowlist)"),
    },
    async ({ device_serial, command }) => {
      try {
        ...
        const output =
          result.length > MAX_SHELL_OUTPUT_BYTES
        ...
        const msg = err instanceof Error ? err.message : String(err);
        return {
          content: [{ type: "text" as const, text: `adb shell failed: ${msg.slice(0, 500)}` }],
        };
      }
    },
  );
    ...
    async ({ device_serial, output_file, time_limit }) => {
      try {
        ...
        if (!output_file.endsWith(".mp4")) {
          return {
            content: [{ type: "text" as const, text: "Error: output_file must end with .mp4" }],
          };
        }
        if (/[^a-zA-Z0-9_\-.]/.test(output_file)) {
          return {
            content: [
              {
                type: "text" as const,
                text: "Error: output_file must be alphanumeric with dashes/underscores only",
              },
            ],
          };
        }
        ...
        if (activeRecordings.has(device_serial)) {
          return {
            content: [
              {
                type: "text" as const,
                text: "Error: recording already in progress on this device. Stop it first.",
              },
            ],
          };
        }
        ...
        if (proc.exitCode !== null) {
          ...
          return {
            content: [
              {
                type: "text" as const,
                text: "Error: screenrecord exited immediately. Is the device screen on?",
              },
            ],
          };
        }
  ...
  server.tool(
    ...
    async ({ device_serial }) => {
      try {
        ...
        if (!recording) {
          return {
            content: [{ type: "text" as const, text: "No active recording on this device." }],
          };
        }
...
const httpServer = Bun.serve({
  ...
  async fetch(req: Request): Promise<Response> {
    ...
    if (url.pathname === "/health") {
      return new Response(JSON.stringify({ status: "ok", service: "mcp-android", port: PORT }), {
